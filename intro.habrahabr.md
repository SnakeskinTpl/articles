...
Мы подключили как модуль файл на Snakeskin (такую бесшовную интеграцию дает, например, плагин для WebPack), импортировав из него namespace **select**, и объявили класс **Select**. При создании инстанса Select, мы выполняем функцию main (в которую был транслирован шаблон main), и присваиваем свойству template результат её работы - для newSelect он будет такой: 
...
Основной мотивацией для написания Snakeskin было желание иметь язык шаблонов с огромными возможностями для повторного использвания кода и который можно было бы использовать на сервере и на клиенте одинаково, без изменения кода. 
Также от него требовалась бесшовная интеграция с фреймворками и библиотеками, которые имеют собственный язык шаблонов (вроде Angular или React -- я же предпочитаю Vue) - и Snakeskin это отлично удаётся.
...
Также, шаблоны можно и нужно предварительно транслировать с помощью плагина для [Gulp](https://github.com/SnakeskinTpl/gulp-snakeskin) или [Grunt](https://github.com/SnakeskinTpl/grunt-snakeskin) и подключать полученные файлы, ну или, как выше - использовать WebPack.

* Генерация статических сайтов: у плагинов есть опция вызывать скомпилированный шаблон и возвращать результат его работы. 
...
либо как модуль (с помощью WebPack, Browserify, RequireJS или любой другой системы модулей).
...
Snakeskin значительно сокращает количество кода, позволяет повторно использовать элементы вёрстки (через наследование, композицию, примеси и т.д.), а Angular делает data-binding. С технической точки зрения SS генерирует шаблон, который потом использует Angular.
