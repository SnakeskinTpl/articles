...
Всем привет! В этом посте хочу рассказать о своей разработке: ЯП для описания текстовых шаблонов - Snakeskin.
Проекту более 3.5 лет, и детские болезни он пережил.
Поэтому с радостью делюсь результатом
...
В 2012 году, когда я работал в Яндексе, вопрос шаблонизаторов часто поднимался на кофепоинтах и в столовых: мы обсуждали достоинства и недостатки различных решений, кто-то рассказывал про свои разработки и т.д. 
У нас в отделе использовался TemplateToolkit2, популярный у Perl разработчиков шаблонизатор. А на клиенте простейший MicroTemplate от Ресига. Также следует отметить, что в то время активно форсились XSLT-подобные решения, но  по ряду причин мы отказались от них. Время от времени мы экспериментировали с другими движками: Handlebars, Dust, Closure Templates, и конечно же свои велосипеды - что привело к наличию целого зоопарка шаблонов в проекте :)

Лично мне нравился Google Closure Templates. Он был близок программисту, т.к. шаблон позиционировался как функция. Плюс, неплохие по тем временам фичи.
Но очень огорчала необходимость править Java файл, чтобы добавить банальный фильтр, да и скорость трансляции была не аховая, т.е. это реально ощущалось.
И мне захотелось сделать собственный Closure Templates, но с блэкджеком и шлюхами, чтобы был написан на JS и был более открытый к модификациям. Также я захотел добавить модель наследования шаблонов основанную на статических блоках, которую подсмотрел в Django Templates (кстати, название Snakeskin - это была косвенная отсылка к Python шаблонизатору :)).

И вот я сел за разработку: прототип написал дня за 2-3 и там был жуткий хардкод на регулярках и строк 600-700 кода. С результатом я немного поигрался, поделился с коллегами и получил некоторый фидбэк.
Далее решил всё это дело стабилизировать, добавить рюшечек и уже начать использовать: после недели разработки запилил версию 2 - это был по сути тот же хардкод на регулярках, но более стабильный и более фичастый.
Поигравшись некоторое время с веткой 2, и выпустив пяток другой обновлений, я сел потирая руки перед монитором с мыслью "настало время всё сделать как надо" и где-то через месяц разработок выпустил 3-ю версию Snakeskin.

Выкинул хардкод, переписал код на ES6 (в то время не было нормальных трансляторов, поэтому я написал на коленке [свой транслятор](https://github.com/kobezzza/NeJS) с жутким хардкодом на регулярках :)), добавил построение дерева при парсинге и много новых фич.

Новая ветка вышла довольно стабильная и представляла из себя Closure Templates на стероидах - я был доволен результатам и стал использовать Snakeskin в своих личных проектах, время от времени выпуская новые обновления и патчи. Через несколько месяцев я познакомился с HAML и Jade, и мне понравился их подход к синтаксису, что решил добавить возможность использования такого синтаксиса.
А после нескольких месяцев плотной разработки я сделал Snakeskin 4, который стал новой вехой в развитии языка и определил дальнейшее развитие. А последующие версии 5 и 6 стали модификацией 4-й версии (т.к. паттерн версионирования у меня - это SemVer, то обновление главной цифры - это просто поломка совместимости, а не "глобальное" обновление).

В общем, SS развился до 6-й версии: фич стало много и стали возникать конфликты между различными директивами - всё было неорганизованно, т.к. язык разрабатывался без начальной спецификации и обсуждений, а по мере нужд разработки и просто хотелок.

SS 6 я использовал довольно долго в самых различных проектах, также эту версию стали использовать мои знакомые и коллеги. В в итоге у меня скопился целый ряд претензий к языку и я решил, что настало время стандартизировать, ввести спецификацию и удалить мусор. Разработка затянулась на 1.5 года (из них активная разработка была где-то 4-5 месяцев), но в итоге получился самый стабильный и продуманный на данный момент релиз - Snakeskin 7, которым я пожалуй даже горжусь :)

Поветствовательная часть на этом кажется заканчивается.

## Что, зачем и почему

Так что же за зверь такой этот Snakeskin (далее SS) и зачем я его написал? Мне нравится определение, что SS - это как CoffeeScript или TypeScript, т.е. просто "сахарный" синтаксис для JavaScript + дополнительные приятные фичи, которые помогают писать код. 
Но в отличии от этих языков, SS имеет явную специализацию - это описание шаблонов, т.е. конечно можно писать на SS приложения целиком, но это просто будет не удобно. SS используется **не вместо, а вместе** с основным языком, например:
...
Мы описали некоторый класс на JS (ES2015) и подключили как модуль файл написанный на Snakeskin (такую интеграцию делает плагин для WebPack), а далее ставим шаблон *main*, как метод класса.

Теперь можно сделать некоторые выводы:

* SS транслируется в JS, который потом простой используется с основным кодом
* В принципе, всё, что можно написать на JS - можно написать и на SS. Но основным призванием SS является генерация шаблонов

Зачем же я написал Snakeskin? Дело в том, что очень хотелось иметь один язык шаблонов с мощной системой code-reuse как на сервере, так и на клиенте. Причём на клиенте использую фреймворк (в данный момент Vue), который, как правило, имеет свой язык шаблонов и SS обязан с ним бесшовно интегрироваться.

## use-case

Теперь рассмотрим основные use-case для Snakeskin:

* Серверная шаблонизация: тут всё просто, подключаем SS как модуль, компилируем шаблон и node.js работает с его функциями, например:
...
Разумеется на практике мы будем использовать фреймворк для приложения, например, Express или Koa, но это не имеет значения.
Также следует отметить, что шаблоны можно транслировать предварительно (например, с помощью Gulp или Grunt) и подключать полученные файлы, или же опять таки - можно использовать подход с WebPack.

* Генерация статических сайтов: с помощью плагинов интеграции (Gulp, Grunt, WebPack и т.д.) мы немедленно вызываем скомпилированный шаблон и сохраняем его результат. Следует отметить что "шаблон для вызова" либо указывается явно, либо будет вычислен по формуле: `%fileName% || main || index || Object.keys().sort()[0];`, где `%fileName%` - имя главного файла-шаблона без расширения (т.е. тот файл, на который мы натравливаем плагин).

* Использование транслированных в JS шаблонов на клиенте: полученные функции можно подключать через внешний тег `<script />` или как модуль (с помощью WebPack), причём SS имеет ряд механизмов для удобной интеграции шаблонов СС с различными фреймворками, например, Angular или Vue.
...
Snakeskin решает проблему "простыни" кода, code-reuse элементов вёрстки (через наследование, композицию, примеси и т.д.), а Angular делает data-binding. С технической точки зрения SS генерирует шаблон, который потом использует Angular.

## Язык

Теперь, когда я описал мотивы и основные use-case, расскажу про сам язык, но цель данного поста не дублирование [документации](http://snakeskintpl.github.io/docs/index-ru.html), а общий обзор.
...
Конечно - это упрощение, но суть проста. Шаблоны - это функции JavaScript, которые, как правило, генерируют текст (в виде строки, DocumentFragment или другом заданном представлении).
...
Плюс такого подхода, что его удобно использовать для генерации текста, который основан на "управляемых пробелах", например, Markdown. Также при таком подходе "блочные" директивы (т.е. которые могут включать в себя текст) обязаны явно закрываться.
