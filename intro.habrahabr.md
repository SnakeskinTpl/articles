...
Основной мотивацией для написания Snakeskin было желание иметь язык шаблонов с развитыми возможностями для повторного использования кода, и который можно было бы использовать на сервере и на клиенте одинаково, без необходимости изменения кода шаблона.
...
Из коробки Snakeskin поддерживает три режима рендеринга: в строку (по умолчанию), в DocumentFragment или в JSX,
а также есть возможность добавить свой рендерер, например, чтобы сгенерировать свой Virtual DOM.
