...
Когда я работал в Яндексе (года четыре назад), одной из самых частых тем для жарких дискуссий на коффепойнтах у нас с коллегами были шаблонизаторы: мы обсуждали достоинства и недостатки существующих решений, некоторые даже разрабатывали свои собственные.

В отделе основным был TemplateToolkit2, популярный у Perl-разработчиков шаблонизатор, на клиенте -- простейший MicroTemplate by Джон Резиг. Ещё в то время активно форсились XSLT-подобные движки, но по ряду причин (обсуждение которых лежит за рамками этой статьи) нам они не подошли. Время от времени мы экспериментировали и с другими: Handlebars, Dust, Closure Templates, плюс свои велосипеды, конечно же… Всё это привело к наличию целого зоопарка шаблонизаторов в проекте.

Моим фаворитом был Google Closure Templates: он был близок мне как программисту, т.к. шаблон позиционировался как функция, результатом работы которой был текст, плюс весьма неплохие по тем временам фичи - но очень огорчала необходимость править Java файл, чтобы добавить какой-нибудь банальный фильтр, да и скорость трансляции была не аховая (это реально ощущалось).

И я захотел сделать свой собственный Closure Templates ~~с блэк-джеком и шлюхами~~: естественно, чтобы был написан на JS и, как следствие, открыт к модификациям без необходимости знать Java. Плюс, мне понравилась модель наследования шаблонов, основанная на статических блоках, которую я подсмотрел в Django Templates (отсюда и название – отсылка к шаблонизатору для Python). Она-то и легла в основу существующей системы наследования.

Прототип я набросал дня за три, и это был жуткий хардкод на регулярках в семь сотен строк кода. С результатом я немного поигрался, поделился с коллегами, получил какой-никакой, но фидбэк – и решил двигаться дальше: я порефакторил это дело, поправил баги, добавил ~~новых~~ возможностей. После недели разработки я запилил версию 2 - это был по сути тот же хардкод на регулярках, но более стабильный и более фичастый. Его уже можно было использовать.

Поигравшись некоторое время со второй версией и выпустив с десяток обновлений, я, потирая руки, сел за компьютер с мыслью "настало время сделать всё как надо", и где-то через месяц разработки выпустил 3-ю версию Snakeskin: выкинул хардкод, переписал код на ES6 (в то время не было нормальных трансляторов, поэтому я ещё написал и [свой транслятор](https://github.com/kobezzza/NeJS) (опять же, с жутким хардкодом на регулярках – да-да, я люблю регулярки)), добавил построение дерева при парсинге и много новых фич.

Версия вышла довольно стабильной и, по сути, представляла собой Closure Templates на стероидах. Я был доволен результатом и стал использовать Snakeskin в своих личных проектах, время от времени выпуская новые обновления и патчи. 

Спустя какое-то время я познакомился с HAML и Jade, мне понравился их подход к синтаксису, и я решил добавить в Snakeskin нечто подобное (результатом стал Jade-like синтаксис). После нескольких месяцев активной разработки я выпустил четвертую версию, которая стала поистине вехой в истории языка, и определила его дальнейшее развитие. Пятая и шестая были не более чем модификацией четвёртой версии, но с ломающими изменениями, которые были необходимы, а так как в качестве паттерна версионирования я использую SemVer, то пришлось апать мажорную версию.

 
SS6 я использовал довольно долго и в самых различных проектах, также его стали использовать мои знакомые и коллеги – в итоге, по прошествии времени, пришло понимание, что так дальше жить нельзя: фич было много, они были достаточно хаотичны, стали проявляться конфликты между различными директивами – причиной этому было то, что язык разрабатывался без какой-либо начальной спецификации, а просто по мере нужд разработки и появления «хотелок».

Я решил, что нужно всё стандартизировать и удалить мусор. Разработка затянулась на 1.5 года (из которых, правда, активная разработка была максимум полгода – сказывалась нехватка свободного времени), но в итоге получился самый стабильный и продуманный на данный момент релиз: Snakeskin 7, и я им искренне горжусь.
